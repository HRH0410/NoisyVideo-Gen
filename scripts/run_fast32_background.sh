...
# 用法示例：
#   bash scripts/run_fast32_background.sh data/input data/output_fast32 42
# 参数：
#   $1 输入目录（默认 data/input）
#   $2 输出目录（默认 data/output_fast32）
#   $3 随机种子（默认 42）

INPUT_DIR="${1:-data/input}"
OUTPUT_DIR="${2:-data/output_fast32}"
SEED="${3:-42}"

# 36 种中排除 3 种慢噪声，再额外跳过 gaussian_noise
FAST_NOISES=(
  poisson_noise impulse_noise speckle_noise
...
echo "[DONE] Fast 32 noises finished."